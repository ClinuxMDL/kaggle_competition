...
class SchNet(chainer.Chain):
    ...
    def __call__(self, input_array, dists, pairs_index, targets):
        ...
        loss = F.mean_absolute_error(out, np.concatenate([targets,targets],axis=0))
    ...
    def predict(self, input_array, dists, pairs_index, **kwargs):
        ...
        concat=F.concat([concat1,concat2], axis=0)
...
optimizer = optimizers.Adam(alpha=init_lr)
...
#         print(cur_epoch)
        ...
        for batch in it:
            ...
            y_total.append(np.mean(y_data.reshape(2,-1,4), axis=0))
