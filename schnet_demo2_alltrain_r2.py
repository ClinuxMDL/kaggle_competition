...
def load_dataset():
    ...
    train = pd.merge(pd.read_csv('../data/train.csv'),
                     pd.read_csv('../data/scalar_coupling_contributions.csv'))
...
if os.path.exists(pretrain_dir):
    serializers.load_npz(pretrain_dir+'/my.model', model)
    print(f"Load the weights of the model: {pretrain_dir}/my.model")
...
def coupling_converter(batch, device):
    ...
    for i, d in enumerate(batch):
        ...
        if with_target:
            list_targets.append(
                d['targets'][['scalar_coupling_constant']].values.astype(np.float32))
...
#         cur_epoch=int(iterator.epoch)+1
#         print(cur_epoch)
...
#         print(y_total)
        y_pred = np.concatenate(y_total).reshape(-1)
#         print(y_pred)
        if self.is_submit:
            self.cur_epoch +=1
            submit = pd.DataFrame()
            ...
            submit.to_csv(output_dir+f'/kernel_schnet_{self.cur_epoch}.csv', index=False)
            serializers.save_npz(output_dir+f'/my_{self.cur_epoch}.model', model)
...
serializers.save_npz(output_dir+'/my.model', model)
print(f"Saving the weights of the trained model into {output_dir}/my.model")
